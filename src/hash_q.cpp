...
HashQ::HashQ(int width):
    width(width), size(1),
    nq_net(width), dq_net(width),
    head_(new HashSegment(width)), tail_(head_.get()) 
{
    tail_.load()->next = std::make_unique<HashSegment>(width);
    aux_tail_ = tail_.load()->next.get();
}

int HashQ::dequeue(int id) {

    auto [key, epoch] = dq_net.get_and_increment(id);
...
void HashQ::enqueue(int value, int id) {
    ...
    auto [key, epoch] = nq_net.get_and_increment(id);
    ...
    if (key % width == width - 1){
        ...
        tmp->next = std::make_unique<HashSegment>(width); 
